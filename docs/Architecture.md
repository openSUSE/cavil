...
|     |- .report.notice.txt.gz           # Cached (gzip-compressed) NOTICE generated by Cavil for this checkout
...
Both the web application and the job queue interact with these files. For a system whose purpose is to
hold legal evidence, the integrity model of that shared storage matters as much as the topology, so it
is worth stating as invariants rather than leaving implicit:

* **Evidence is content-addressed and is never overwritten in place.** A checkout lives at
  `<name>/<verifymd5>`, where `verifymd5` is a checksum of the source itself. Identical source always
...
* **One pipeline owns a package at a time; within it, indexing writers are parallel but controlled.**
  ...
  builds the tree, index clears the stale documents). The web application never writes an existing checkout -
...
* **The source is the evidence; everything else is a derived cache.** The imported source is
  ...
  PostgreSQL, and the cached `.report.*` documents are all regenerable from it. PostgreSQL is the
  source of truth for review decisions and the curated pattern corpus; the filesystem holds the
  evidence and these caches.
* **Reindexing invalidates and rebuilds; it never patches.** Re-running the pipeline recreates the
  derived rows from scratch and recomputes the stored resolutions wholesale, and the SBOM is
  regenerated by its own job - so a report always reflects the current derived state rather than an
  accumulation of in-place edits. This is the same disposability the automated-resolution section
  below relies on.
...
Two more background jobs exist outside this chain. Derived documents are built on demand, when a reviewer asks for them
on the report page or a client requests one that has not been cached yet.

8. `documents`: Creates every derived document the registry lists (the SPDX SBOM and the NOTICE).
...
human should look at first, and the bot marks product imports right at the bottom of it. It shifts an import up or
down *within* the incoming band and never out of it, so an urgent request is still handled first among the packages
arriving that afternoon, and the least urgent one still goes in ahead of every reindex.
...
A legal report also flags pairs of licenses that cannot normally be combined into one work. The verdicts come
straight from the OSADL license compatibility matrix, restricted to the licenses actually present in the
package and shown verbatim; Cavil never invents or reinterprets a verdict. On its own that is noisy: a package
that bundles many dependencies can flag dozens of pairs, and almost all of them are false alarms, because two
licenses merely shipped side by side in one archive are an aggregation, not a combined work. Only code that is
genuinely combined (linked, compiled together, or merged into one file) creates a real conflict.

Cavil cannot tell from static analysis whether two components are actually combined, so it does not try to
decide. Instead it ranks each flagged pair by how likely a real combination is, so a reviewer sees the handful
that matter first and the aggregation tail sinks to the bottom. Nothing is ever hidden: a legal reviewer must
be able to see every possible incompatibility, so weak and aggregated pairs stay on the matrix, just ranked
down and clearly labelled.
...
2. **How trustworthy the evidence is.** A real license match outranks a folded snippet, which outranks the
   ...
   because a reviewer should see the possibility, but it never displaces stronger evidence.
3. **How close the two licenses sit.** Both licenses in one file is the strongest proximity signal, then the
   same directory, then a shared parent directory (the deeper the shared directory, the stronger), and finally
   two licenses that meet only at the package root, which is the classic aggregation shape.
...
The SBOM is one of several documents derived from an indexed package. They are listed in a registry
(`Cavil::Model::Packages`), and the `documents` job builds every entry in one pass, either from the download control on
the report page or from a request for one that has not been built yet. Adding a format is a registry entry plus its
generator: the job, the state the report page polls, the download routes and the report UI are all driven off the list.

Each document is cached next to the package checkout, gzip-compressed where that helps - the SBOM's JSON is highly
repetitive and compresses roughly ten-fold, which matters at the scale of many cached reports. A gzipped document is
served over the REST API untouched to clients that accept gzip (essentially all of them) and decompressed on the fly
for the rest; if a request arrives before it has been generated, the API returns a `408` status until it is ready. The
creator identity, document namespace and `LicenseRef` namespace are all set in the instance configuration.

See the [User API documentation](UserAPI.md#retrieve-spdx-reports) for the exact endpoint and an example document.

### NOTICE

The second document is a NOTICE: the attribution artifact a redistributor has to ship, grouped by license. Each section
names a license present in the package, lists the copyright notices harvested from the files that license was matched
in, and then reproduces its terms. Notices from files no license was matched in are kept in a final section rather
than dropped. A license text is reproduced once however many expressions name it, so a package carrying three GPL
variants does not ship the same text three times.

Terms resolve in one order, shared by every document so two of them cannot disagree about a license:

1. The bundled SPDX text, one per identifier in the expression. It only counts if the identifiers cover the whole
   expression: `MIT AND Vendor-Thing-1.0` prints MIT's terms but is still a gap, because half of an `AND` is not the
   license.
2. The pattern a curator marked as the license's full text (`license_patterns.full_license_text`). This is how a
   license with no SPDX identifier gets terms at all. One pattern per license, and never one containing a `$SKIP`
   wildcard, since the text is printed exactly as stored.
3. Nothing - the license is a gap.

What a match happened to cover in a package is deliberately not on that list. It shows an identifier applies, but a
pattern is usually a short snippet rather than a license, so printing it would pass a fragment off as the terms. The
SBOM still uses it for a `CustomLicense`, where it is labelled as text found at a location.

Cavil cannot resolve terms for every license, and quietly omitting them would be worse than saying so, so the document
opens with a completeness statement naming every license whose text is missing and why. The legal-documents panel on
the report page points at the files a curator would take the missing text from.

Two details decide whether the grouping is correct. A notice is harvested against the *original* file, because the
post-processed copy wraps lines and splits a notice from the holder it names, while a match is recorded against the
file as *scanned*; the join undoes that, or every wrapped and markup-stripped file silently loses its notices. And the
join runs in the database, streamed back through a cursor, because a heavily vendored package has millions of
(notice, file) pairs that must never all be in memory at once.
...
* `license_patterns` - the patterns themselves, including their license name, risk, global-vs-package scope, a
  ...
  `-Unspecified` makes the whole license one. A `full_license_text` flag marks the one pattern per license whose text
  *is* the license in full, which is how a license with no SPDX identifier gets terms to reproduce.
