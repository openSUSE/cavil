...
|     |- .report.spdx.json               # Cached SPDX SBOM generated by Cavil for this checkout
...
* **Component name and version** — taken from the package and its spec file.
...
* **Cryptographic hashes** — a SHA-512 checksum for every file, and a verification digest for the package as a whole.
...
* **Dependencies** — the files and subcomponents contained in the package, expressed as SPDX relationships with an
...
The package Cavil reviewed is the primary component. Cavil also unpacks the archives inside it (upstream tarballs, for
example), and each unpacked archive is represented as a **subcomponent**, with the files it contains linked to it. Every
containment relationship is marked as *complete*, because Cavil has scanned the entire unpacked tree and therefore knows
the full file list.

What Cavil does **not** currently do is resolve *bundled third-party libraries* into separate, individually-described
components — a source tree that vendors a copy of another project is described by its files, not as a distinct upstream
dependency. Detecting vendored dependencies reliably across the huge variety of packaging conventions is an unsolved
problem, so this is a deliberate, documented limitation rather than an oversight. The place where such information would
be attached is isolated in the generator, so if richer subcomponent data becomes available in future (for example from
the build service itself), it can be plugged in without redesigning the export.

Similarly, the BSI "executable / archive / structured" component properties are approximated from each file's detected
type; where the underlying data cannot determine a property, it is omitted, which the guideline explicitly permits.
...
cached next to the package checkout as `.report.spdx.json` and served over the REST API; if a request arrives before the
SBOM has been generated, the API returns a `408` status until it is ready. By default SBOMs are produced on demand, but
an instance can be configured to generate one automatically whenever a package is indexed. The creator identity,
document namespace and `LicenseRef` namespace are all set in the instance configuration.
