...
in, and then reproduces its terms - the bundled SPDX text for an identifier, or the text the match was made against for
a license Cavil knows by name only. Notices from files no license was matched in are kept in a final section rather
...
Cavil cannot resolve terms for every license it detects, and a NOTICE that quietly omitted those would be worse than
none, so the document opens with a completeness statement naming every license whose text is missing and why. That is
what makes the remaining identifier-curation work visible per package.
