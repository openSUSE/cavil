...
my $detect_id  = $indexed_job->info->{children}[0];
my $detect_job = $t->app->minion->job($detect_id);
is $detect_job->task,           'detect_components', 'right task';
is $detect_job->info->{state},  'finished',          'job is finished';
is $detect_job->info->{result}, undef,               'job was successful';
my $analyze_id  = $detect_job->info->{children}[0];
my $analyze_job = $t->app->minion->job($analyze_id);
is $analyze_job->task,           'analyze',  'right task';
is $analyze_job->info->{state},  'finished', 'job is finished';
