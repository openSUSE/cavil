...
sub isnt_part_of {
  my ($first, $second) = @_;
  ok !lic($first)->is_part_of(lic($second)),
    qq{"$second" is not part of "$first"};
}

sub is_similar_to {
  my ($first, $second) = @_;
  ok(
    lic($first)->is_similar_to(lic($second)),
    qq{"$second" is similar to "$first"}
  );
}

sub isnt_similar_to {
  my ($first, $second) = @_;
  ok !lic($first)->is_similar_to(lic($second)),
    qq{"$second" is not similar to "$first"};
}
...
is_deeply $l->canonicalize->tree, {license => 'AGPL-3.0-only'},
  'right canonicalized structure';
...
ok !$l->exception,  'no exception';
is $l->error, undef, 'no error';
is_deeply $l->tree, {license => 'GPL-3.0-with-Qt-Company-Qt-exception-1.1'},
  'right structure';
...
is_deeply $l->canonicalize->to_string,
  'AGPL-3.0-only and Artistic-1.0 and Ruby', 'right canonicalized string';
...
# Parentheses
$l = lic(
  '(LGPL-2.1-only or LGPL-3.0-only) and (GPL-3.0-or-later or GPL-2.0-only)');
ok !$l->normalized, 'not normalized';
ok !$l->exception,  'no exception';
is $l->error, undef, 'no error';
$ast = {
...
$ast = {
  ...
  right => {
    left => {
      ...
      op    => 'or',
      right => {
        left  => {license => 'Artistic-1.0'},
        op    => 'and',
        right => {license => 'Apache-1.1'}
      }
    },
    op    => 'and',
...
is_deeply $l->tree, $ast, 'right structure';
is $l->to_string, 'Ruby and (GPL-1.0-or-later or (Artistic-1.0 and Apache-1.1))'
  . ' and AGPL-3.0-only', 'right string';
is $l->canonicalize->to_string,
  '((Apache-1.1 and Artistic-1.0) or GPL-1.0-or-later)'
  . ' and AGPL-3.0-only and Ruby', 'right canonicalized string';
...
$ast = {
  left => {
    left  => {license => 'Ruby'},
    op    => 'and',
    right => {license => 'GPL-1.0-or-later'}
  },
  op    => 'or',
  right => {
    left  => {license => 'Artistic-1.0'},
...
is_deeply $l->tree, $ast, 'right structure';
is $l->to_string,
  '(Ruby and GPL-1.0-or-later) or (Artistic-1.0 and AGPL-3.0-only)',
  'right string';
is $l->canonicalize->to_string,
  '(AGPL-3.0-only and Artistic-1.0) or (GPL-1.0-or-later and Ruby)',
  'right canonicalized string';
...
is_deeply $l->canonicalize->to_string, 'SUSE-Freeware',
  'right canonicalized string';
...
is_deeply $l->canonicalize->to_string, 'Apache-2.0+',
  'right canonicalized string';

# Bad parentheses
$l = lic('(LGPL-2.1-only or LGPL-3.0-only and (GPL-2.0-only or GPL-3.0-only');
ok !$l->normalized, 'not normalized';
is $l->error, 'Invalid license expression: (LGPL-2.1-only or LGPL-3.0-only'
  . ' and (GPL-2.0-only or GPL-3.0-only', 'right error';
...
ok !$l->normalized, 'not normalized';
is $l->error,
  'Invalid license expression: %{license_apache2} and %{license_mit}',
  'right error';
is_deeply $l->tree, undef, 'no structure';
...
is_part_of 'Apache-1.0 or Apache-2.0 and Ruby',
  'Apache-1.0 or Apache-2.0 and Ruby';
...
is_similar_to 'Ruby and GPL-1.0+ or Artistic-1.0 and AGPL-3.0',
  ' (AGPL-3.0 and Artistic-1.0) or (Ruby and GPL-1.0+)';
