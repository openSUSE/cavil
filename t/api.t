...
subtest 'Review grace period' => sub {
  my $pkgs   = $t->app->packages;
  my $config = $t->app->config;

  # Manually review package 1
  my $pkg = $pkgs->find(1);
  ...
  subtest 'No grace period (result visible immediately)' => sub {
    $config->{review_grace_period} = 0;
    $t->get_ok('/package/1' => {Authorization => 'Token test_token'})
      ->status_is(200)
      ->json_is('/state',  'acceptable')
      ->json_is('/result', 'Reviewed ok');
    $t->get_ok('/api/1.0/package/perl-Mojolicious')->status_is(200)->json_is('/requests/0/state', 'acceptable');
  };

  subtest 'Within grace period (result held back)' => sub {
    $config->{review_grace_period} = 600;
    $t->get_ok('/package/1' => {Authorization => 'Token test_token'})
      ->status_is(200)
      ->json_is('/state', 'new')
      ->json_hasnt('/result')
      ->json_hasnt('/reviewing_user');
    $t->get_ok('/api/1.0/package/perl-Mojolicious')->status_is(200)->json_is('/requests/0/state', 'new');
  };

  subtest 'Grace period elapsed (result visible again)' => sub {

    # Backdate the review beyond the window (clock simulation only)
    $t->app->pg->db->query("UPDATE bot_packages SET reviewed = NOW() - INTERVAL '20 minutes' WHERE id = 1");
    $config->{review_grace_period} = 600;
    $t->get_ok('/package/1' => {Authorization => 'Token test_token'})
      ->status_is(200)
      ->json_is('/state',  'acceptable')
  ...
  subtest 'Auto-accepts are held back too' => sub {

    # Simulate an auto-accept: freshly reviewed, but with no reviewing user
    my $auto = $pkgs->find(1);
    $auto->{state}            = 'acceptable';
    $auto->{reviewing_user}   = undef;
    $auto->{review_timestamp} = 1;
    $auto->{result}           = 'Accepted because of low risk';
    $pkgs->update($auto);
