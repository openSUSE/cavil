...
// estimated risk. The "report_notice" fixture builds two synthetic versions of
// one package through the real pipeline: version 1 is accepted, version 2 (id 2)
// adds eight brand-new unresolved files.
t.test('Cavil UI - new unresolved matches badges', skipUnlessOnline, async t => {
  ...
  try {
    ...
    await t.test('every new unresolved file is badged in the Risk 9 section', async t => {
      await page.waitForSelector('#filelist-snippets .risk-unresolved-new');
      const rows = page.locator('#filelist-snippets .risk-unresolved-item');
      const badges = page.locator('#filelist-snippets .risk-unresolved-new');
