...
sub run {
    ...
    $db->select('license_patterns', '*', {}, {order_by => 'id'})->hashes->each)
  {
    ...
    if ($result->{conflict}) {
      ...
      say
        "Pattern 1: #$cattern->{id} ($cattern->{license} package: $cattern->{packname} risk: $cattern->{risk} eula: $cattern->{eula} trademark: $cattern->{trademark} opinion: $cattern->{opinion})";
      say
        "Pattern 2: #$pattern->{id} ($pattern->{license} package: $pattern->{packname} risk: $pattern->{risk} eula: $pattern->{eula} trademark: $pattern->{trademark} opinion: $pattern->{opinion})";
      my $t1 = $pattern->{pattern} . "\n";
